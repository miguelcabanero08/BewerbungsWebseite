// React Bits — DepthCarousel (JS + CSS Variante), leicht angepasst:
// - `icon`-Prop + Fallback-Karte, falls ein `item.image` (noch) nicht existiert
//   (z.B. Poster/Cover, die erst später ergänzt werden) — analog zu ImagePlaceholder.jsx.
...
const DepthCarousel = ({
  ...
  const onCardClick = useCallback(
    (index) => {
      ...
      // Karte ist schon im Fokus (vorne, zentriert): Klick zoomt statt erneut zu navigieren.
      if (index === active) {
        const item = data[index]
        if (item?.image && !erroredIdx.has(index)) {
          openLightbox({ type: 'image', src: item.image, alt: item.alt })
        }
        return
      }
      setFocus(index, true)
    },
    [setFocus, active, data, erroredIdx, openLightbox]
  )
    ...
    >
      <div className="depth-carousel__stage" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            {erroredIdx.has(i) || !item.image ? (
              <div className="depth-carousel__fallback">
                <Icon size={28} />
                {item.alt && <span className="depth-carousel__fallback-label">{item.alt}</span>}
              </div>
            ) : (
              <>
                <img
                ...
                />
                {active === i && (
                  <div className="depth-carousel__zoom-hint">
